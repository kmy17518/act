...
**BatchNorm train/eval gap (all precisions).** In eval mode — running statistics, which is how the checkpoint is served — the same weights score L1 **0.175 instead of 0.112** (+56 %). Re-estimating the running statistics from the current weights over all 48 camera passes does not help (0.178), so this is not EMA lag: RoboAgent's backbone normalizes each camera pass with that camera's own batch statistics, whereas a single running mean/variance can only hold a mixture over the three cameras, so eval-mode activations differ from anything seen in training. The frozen-BatchNorm baselines have no such gap: on the `my` (unconditioned) and `lang_optimized` (clip_film identity/random) step-5,000 checkpoints the backbone's only normalization layers are `FrozenBatchNorm2d`, predictions for the same 780 samples are bit-identical (max |Δ| = 0) when the other 780 samples in the batch are exchanged, and eval-mode L1 is below train-mode L1 (0.1082 vs 0.1164, 0.1187 vs 0.1234, 0.1270 vs 0.1365; dropout off), whereas the per-camera MT-ACT checkpoint with batch statistics moves by up to 0.12 under the same exchange (`/tmp/act-lang-probe/frozen_bn_eval.py`). For a deployable MT-ACT reproduction the cameras should share BatchNorm statistics in training as well (one backbone call over all camera images — a deliberate deviation from RoboAgent's per-camera loop), or the backbone should keep frozen/ImageNet statistics; `--backbone-camera-batch` (commit `8eb166f`) implements the former.

*Original design or our artifact?* The mechanism is RoboAgent's as released: `detr_vae.py` runs the shared BatchNorm ResNet once per camera, `train.py` computes its validation loss under `policy.eval()`, and `evaluate.py` deploys with `policy.eval()` and batch size 1, i.e. with the mixed running statistics. Its magnitude depends on how different the cameras' statistics are (our head camera vs two wrist cameras; RoboAgent had three fixed views plus a wrist camera and batch 64), so RoboAgent's own gap may have been smaller, but it exists by construction. Confirmation on our checkpoints (`/tmp/act-lang-probe/bn_camera_eval.py`, same 16 batches, TF32 forward, no dropout):

| Weights \ BatchNorm statistics at evaluation | per-camera batch statistics | joint (all-camera) batch statistics | running statistics (eval mode) |
| --- | --- | --- | --- |
| per-camera-trained TF32 run (RoboAgent's passes) | **0.1121** | 0.1654 (+48 %) | 0.1747 (+56 %) |
| camera-batched-trained `cambatch-…-bf16bb` run | 0.1645 | **0.1163** | 0.1170 (+0.6 %) |
...
Two 240 px runs with `--backbone-camera-batch` (commit `8eb166f`; now the MT-ACT recipe default, `ACT_CAMERA_BATCH=0` restores RoboAgent's per-camera passes), otherwise identical to the per-camera runs above, stopped at 5,000 steps by the watcher (exit 130, `step_00005000.pt` saved, W&B runs finished):
...
| **MT-ACT per-camera statistics (`--backbone-norm batch_per_camera`), TF32** | **0.1119** | 0.1121 | 0.1227 | 0.1234 |
| MT-ACT per-camera statistics, `bf16-backbone` | 0.1117 | 0.1120 | 0.1226 | 0.1235 |

Shared statistics remove the gap (eval 0.1162 vs batch-statistics 0.1161) and cut the served-mode error by a third (0.175 → 0.116) at identical cost; the per-camera runs' lower training-log L1 (0.1234 vs 0.1280) was the flattering effect of a normalization that serving cannot reproduce — unless the running statistics are kept per camera: `--backbone-norm batch_per_camera` (next section; rows added 2026-09-19 with the same `/tmp/act-lang-probe/percam_eval.py` protocol) keeps RoboAgent's per-camera training computation — its weights are bitwise identical to the RoboAgent-faithful run's (310/310 parameter tensors) — and serves at 0.1119, i.e. the training regime's 0.1121, 3.7 % better than the joint statistics of camera batching. Served, the from-scratch MT-ACT variants thus rank per-camera statistics (0.112) < joint statistics (0.116) < RoboAgent-faithful mixed statistics (0.175), between the ImageNet-initialized baseline (0.108) and the CLIP FiLM variants; `bf16-backbone` stays within 0.7 % of TF32 in every variant. Training losses on 24,960 samples at 1.7 % of the schedule; no rollout or convergence claim.

*Reproducibility (2026-09-19).* A second, independently launched `batch_per_camera` TF32 run (`outputs/turning-on-radio-mt-act-percam-240px-bs1560-300k-opt20260917-rerun`, W&B `actradio-mtact-percam-240px-opt20260917-rerun`, GPU 1 / cores 0-29, 0.499 s/step, 196 GiB, stopped after step 5,006 by the watcher, log `/tmp/dev/logs/act-radio-mt-act-percam-240px-300k-opt20260917-rerun.log`) reproduced the first one **bitwise**: all 490 state tensors including running statistics equal, training-log L1 4,901–5,000 0.12339 in both, and the same 0.1119 / 0.1121 / 0.1227 evaluation. The compiled, autotuned, spawn-loader pipeline is deterministic run-to-run at this batch, so differences between the runs in these tables are not launch noise.

### Per-camera running statistics (`--backbone-norm batch_per_camera`) — 2026-09-18

Commit `6ad45ce` adds the other fix as a model option (b1k.md "Per-camera running statistics"): RoboAgent's per-camera passes and training computation unchanged, one set of BatchNorm running statistics per camera, selected before each pass, so serving normalizes each camera as training did. Two 240 px runs (`ACT_BACKBONE_NORM=batch_per_camera`, "-percam"), otherwise identical to the per-camera runs above, stopped at 5,000 steps by the watcher (exit 130, `step_00005000.pt` saved, W&B runs finished):
