...
- **Initialization (`--film-init random|identity`):** `random` (default) keeps `nn.Linear`'s default initialization; `identity` zeroes the projection weight and bias so beta = gamma = 0 and every FiLM layer starts as the identity on the (non-negative) block output — the conditioned network then computes exactly what the unconditioned one does at step 0 (the paired diagnostic below verified this). The choice is saved as `model_config['film_init']` (absent in older checkpoints = `random`); resume adopts it and rejects a contradictory flag. `--film-init` without `clip_film` is an error.
- **Memory (`--film-recompute`, default on):** conditioned training activation-checkpoints each residual-block/FiLM pair with `use_reentrant=False`. The frozen BatchNorm path is deterministic for recomputation. This trades compute for activation memory without changing the physical optimizer batch, FP32 arithmetic, precision flags, or gradient accumulation. `--no-film-recompute` stores the activations instead (faster when the batch fits; a runtime choice, not saved). The `none` path is unchanged. Qualify the intended full batch on the reserved GPU; no maximum batch is implied by CPU tests. The throughput options below (`--frame-cache`, `--compile`, `--channels-last`, `--fast-maxpool`, ...) apply unchanged to conditioned training; the compiled backbone region includes the FiLM layers.
...
## Optional MT-ACT reproduction (`--language-conditioning mt_act`)

Reproduces the language-conditioned policy of RoboAgent's MT-ACT ([Bharadhwaj et al., CoRL 2023](https://arxiv.org/abs/2309.01918); code `robopen/roboagent`) on the BEHAVIOR adapter. Everything that is an architecture or training-recipe property follows the released code; data-bound quantities (three cameras, 25-D proprioception, 23-D actions, chunk 100) follow this adapter so the losses stay comparable with the other radio runs.

- **Text encoder (`--language-encoder minilm`, default for `mt_act`):** frozen `sentence-transformers/all-MiniLM-L6-v2` (pinned revision `1110a243…`), masked mean pooling of the last hidden state, L2 normalization, truncation at 256 word pieces — the sentence-transformers semantics RoboAgent's `constants.TEXT_EMBEDDINGS` were produced with. Verified: encoding RoboAgent's own task strings reproduces its shipped 384-d vectors to 1.3e-7 (cosine 1.000000) for every prompt checked. `--language-encoder clip` remains available for either conditioning mode; the encoder name and dimension are recorded as `model_config['language_encoder']` / `language_cache` and enforced on resume and serving. Use `--prompt-source task_description` for MT-ACT's natural-language instructions.
- **Learned projection and language token:** one `proj_text_emb: Linear(384, hidden_dim)`; its output conditions the ResNet through FiLM *and* is appended to the transformer encoder input as a third extra token after the latent and proprioception tokens, with a third learned position embedding (`additional_pos_embed(3)`).
- **FiLM placement:** inside the residual branch of every `BasicBlock` in ResNet stages 2–4 (`layer2`–`layer4`; stage 1 and the stem are unmodulated): `relu(identity + (1 + gamma) * bn2(conv2(...)) + beta)`, with one `Linear(hidden_dim, num_blocks * 2 * planes)` generator per stage (gamma first, beta second, as `resnet_film.py`). `--film-init identity` zeroes the generators. `--film-recompute` does not apply (nothing to recompute at these sizes).
- **Style encoder:** `[CLS, actions]` only — no proprioception token (`pos_table(num_queries + 1)`, no `encoder_joint_proj`).
- **No one-hot task input:** the adapter still appends the one-hot category to qpos (it selects the cached embedding, `adapter_config['task_conditioning']` stays `onehot`), but `model_config['state_dim']` is the 25 proprioception values and the network ignores the trailing entries; serving is unchanged.
- **Backbone:** RoboAgent's `build_film_backbone` constructs ResNet-18 without ImageNet weights and with trainable `BatchNorm2d`; pass `--no-pretrained-backbone --backbone-norm batch` for the faithful reproduction (`--backbone-norm` is saved model configuration; `frozen` keeps the upstream ACT frozen statistics and works with either conditioning mode). BatchNorm running statistics live in the checkpoint like any buffer; bit-exact resume is covered by the tests.
- **Camera passes (`--backbone-camera-batch`, saved model configuration):** ACT runs the shared backbone once per camera. With trainable BatchNorm that normalizes every camera pass by *its own* batch statistics, while the single running mean/variance used in eval mode can only hold a mixture over the cameras — the served network then computes something it never saw in training (measured: eval-mode L1 0.175 vs 0.112 with batch statistics at step 5,000; re-estimating the running statistics does not help, b1k_runs.md). `--backbone-camera-batch` runs the backbone once over the camera-major batch of all camera images, so the statistics are shared across cameras in training exactly as they are at serving time (eval-mode L1 0.116 vs 0.116 with batch statistics). For frozen-BatchNorm models the option is a bit-exact reformulation of the per-camera loop (tests cover sine and learned positions, unconditioned and `clip_film`), and it costs no time at batch 1560 (0.497 vs 0.499 s/step). It deviates from RoboAgent's per-camera loop; the MT-ACT launch recipe enables it by default (`ACT_CAMERA_BATCH=0` restores the per-camera passes). CNNMLP has one backbone per camera and rejects the flag.
- **Per-camera running statistics (`--backbone-norm batch_per_camera`, saved model configuration):** the other fix, keeping RoboAgent's per-camera passes and training computation unchanged. `PerCameraBatchNorm2d` (`detr/models/backbone.py`) normalizes with the current pass's batch statistics and shares the affine weight/bias like `nn.BatchNorm2d`, but keeps one running mean/variance/`num_batches_tracked` per camera; `DETRVAE` selects the camera before each backbone pass (`Joiner(..., camera=)` → `BackboneBase.select_camera`), so eval mode normalizes each camera exactly as training did — domain-specific BatchNorm with the cameras as domains. State-dict keys are `running_mean_<c>` etc.; a single-statistics state dict (ImageNet weights, a `--backbone-norm batch` checkpoint) loads by copying its statistics to every camera. Exclusive with `--backbone-camera-batch` (joint statistics need one pass; per-camera statistics need one pass per camera); CNNMLP rejects it. Same cost as the per-camera passes (0.50 s/step TF32 at 240 px; the compiled body specializes once per camera index). Requires the camera identity at inference, which the fixed camera order provides; serving is unchanged. `scripts/b1k/recalibrate_camera_batchnorm.py` converts an existing per-camera-pass `batch` checkpoint without retraining: it broadcasts the weights into the per-camera layout, re-estimates each camera's statistics as the cumulative average over training batches (BatchNorm in train mode only, no gradients) and writes a checkpoint of the same type (full checkpoints remain resumable; `--evaluate` reports the eval-mode L1 before and after). On the 240 px TF32 step-5,000 checkpoint 16 batches take 7 s and move the served L1 from 0.175 to 0.112 (b1k_runs.md).
- **Not reproduced:** RoboSet's semantic (inpainting) augmentation, four camera views, H=20 chunks with temporal aggregation, the 80/20 trajectory validation split and best-validation checkpoint selection; the throughput options below apply unchanged (the FiLM body is part of the compiled backbone region).

`scripts/b1k/run_radio_mt_act_300k.sh` is the launch recipe (`ACT_IMAGE_SIZE` 96 — the Diffusion Policy baseline's resolution — or 240, this adapter's ACT default; batch 1560, the radio optimizer settings). CPU coverage: `tests/test_b1k_language.py` (`ACT_TEST_REAL_MINILM=1` adds the cached real-encoder check against RoboAgent's first shipped vector).

**Throughput of the MT-ACT architecture** (batch 1560, 240×240, `--compile regions-autotune`, 25-step probes and live runs, steady-state median): the MT-ACT-specific modules are free — the three FiLM generators, `proj_text_emb` and the extra encoder token add well under 1 ms — but trainable `BatchNorm2d` is not: the unconditioned/CLIP recipes' frozen BatchNorm is a per-channel affine that Inductor fuses into the neighbouring elementwise kernel and that saves nothing for backward, whereas batch-statistics BatchNorm adds a reduction pass over every convolution output in forward, two more in backward, and keeps those inputs for backward. Measured: TF32 **0.50 s/step, 196 GiB** vs 0.45 s/step and 157 GiB for the same network with `--backbone-norm frozen` (a speed-only diagnostic — an unnormalized from-scratch ResNet is not a training option). That cost is memory-bandwidth-bound, so halving activation bytes is the lever: `--autocast bf16-backbone` (bf16 inside the ResNet only; statistics, running statistics, FiLM generators' outputs and everything after the backbone stay fp32/TF32) gives **0.43 s/step, 156 GiB**; `--autocast bf16` (whole forward in bf16 with fp32 heads, latent distribution and losses; the 195-token encoder is memory-bound at this batch too) gives **0.34 s/step, 103 GiB**. Both are precision changes, not architecture changes — b1k_runs.md records their step-5000 losses next to the TF32 run. Batching the three camera passes into one backbone call was considered and not adopted: it would move the same bytes through the same kernels (no measurable gain at this batch) while changing which samples share BatchNorm statistics (RoboAgent normalizes each camera pass separately).

## Goal-image conditioning (`--regime`, `--goal-fusion`)

The `goal` branch (from `my@6cf7718` plus the language ports above) adds goal-image conditioning as configuration presets on the unchanged base policy. Every run declares a **conditioning regime**, the modality availability of the experiment: `--regime none` (N: observation/proprioception only), `language` (L), `image` (I) or `image_language` (LI). The regime validates the mechanism flags (`--language-conditioning`, `--goal-fusion`, `--task-onehot`) and fills defaults (L/LI: `mt_act`; I/LI: late fusion of the head goal), so a misconfigured run fails instead of silently training another condition. With a regime the one-hot task id stays **out of the state** in every condition (`adapter_config['task_conditioning'] = 'none'`, `state_dim = 25`): task identity reaches the network only through language or the goal image; `--task-onehot` with a regime is a separate task-ID experiment and is logged as such. Without `--regime` the historical behaviour stands (one-hot on). The dataset task id travels in the batch as its own column (`task_id`) for prompt selection, so language no longer depends on the one-hot; `b1k_language.language_for_tasks` replaces `language_for_qpos` in the trainer and server.

**Data contract.** Samples are `(images, qpos, actions, is_pad, goal, task_id)`. `goal` holds one uint8 image per selected view (`--goal-views`, camera names; default `zed_link`, the head) with **no observation-history axis**, looked up per episode from a table built once at dataset construction and pickled to the loader workers (400 episodes × 240×240 ≈ 69 MB per view). `--goal-source episode_last` (default) decodes the last frame of the episode's own camera stream — the settled terminal state; it works on any LeRobot v3 root — and `goal_key` decodes the dataset's static `observation.goal_rgb.<camera>_camera_0` clip (the radio skill-segment datasets ship both, with identical content). Goal frames go through `preprocess_image` and are rounded to uint8 exactly like the frame cache, so the goal path and the cached camera path agree to 0.5/255. The adapter records `goal_views`, `goal_source`, `goal_video_keys` and the wire keys `goal::<camera observation key>`; `run.json` and every checkpoint carry a `conditioning` block (regime, language implementation/encoder/prompt source, goal fusion/views/encoder/source/sampling policy, task one-hot, pretrained encoder, source commit, dataset root/episodes/frames).

**Early fusion (`--goal-fusion early`).** The head camera's image is channel-stacked with its goal image, `[current; goal]`, before the ResNet stem — BridgeData V2's ACT conditioning mechanism (Appendix B.3). Instead of a second backbone, `detr/models/backbone.py::PairedConv2d` replaces the shared stem's `conv1` by one 6-channel convolution whose weights are `[W_obs, W_goal]`: `W_obs` keeps the ImageNet weights and `W_goal` (`goal_weight`) starts at **zero**, so at initialization the paired stem computes exactly what the base stem does and a zero goal is an exact "absent goal" (`goal_valid=False` zeroes the goal half). Other cameras (and camera-batched passes, which pad the goal half with zeros) run the plain stem. Both halves receive the ImageNet normalization before stacking; they are never passed through a three-channel-only transform. The zero initialization and the shared-stem construction are our controlled engineering choices, not claims about BridgeData's unpublished details; the two-goal test shows the zero stem needs a larger backbone learning rate than late fusion to become goal-sensitive quickly — do not expect immediate goal sensitivity from it.

**Late fusion (`--goal-fusion late`).** The goal image runs through the current RGB backbone (`--goal-encoder shared_base`; `separate_base` uses an architecturally identical copy, a capacity/sharing ablation), is projected with the same `input_proj`, and its **full spatial token grid** is appended to the transformer encoder memory after the current-view tokens: `[latent, proprio, (MT-ACT task token), current-view tokens, goal-view tokens]`. The goal tokens keep the sine spatial positions and get a learned per-view goal identity (`goal_role_embed`, zero-initialized) on the positional stream; current-view tokens are untouched. `goal_valid` builds a key-padding mask over the goal columns, which the tests show reduces the network to the base model exactly. Pooling the grid to one token is a further compression ablation, not the default. With language conditioning the goal pass runs the FiLM layers at the identity (gamma = beta = 0; `--language-on-goal-encoder` modulates it too and is saved as `language_on_goal_encoder`); the observation pass still rejects a missing embedding. Goal modules are created after every base module, so with a fixed seed the base parameters are initialized identically with and without goal conditioning.

Claims: the early family mirrors BridgeData V2's ACT conditioning mechanism inside this training infrastructure (a conditioning mirror, not a complete baseline reproduction); LI compositions (MT-ACT-style language + either goal path) are our compositions of published mechanisms, not a verified published joint baseline; the cVAE, action decoder, loss, chunk, optimizer and throughput options are unchanged in every preset.

Two-task mixture and smoke recipe: `scripts/b1k/merge_lerobot_roots.py` merges the nav-goal and pickup-goal skill-segment roots into `/tmp/dev/datasets/2026-challenge-demos-radio-navpickup-goal` (400 episodes, two tasks; camera videos hard-linked from the challenge demos so the 240 px frame cache validates unchanged), and `scripts/b1k/run_navpickup_conditioning_smoke.sh` launches the `vanilla | none | language | image-early | image-late | image_language-early | image_language-late` conditions (5,000 steps, batch 1560, the radio optimizer/throughput settings; see b1k_runs.md). Serving: goal-conditioned checkpoints take the goal image under `goal::<camera observation key>` in the request, or a fixed `--goal-image CAMERA=PATH` file per view; the handshake reports `regime`, `task_conditioning`, `goal_fusion`, `goal_views`, `goal_observation_keys` and `goal_source`.

CPU coverage: `tests/test_b1k_goal.py` — goal table = episode last frame / dedicated stream; goal-free configuration is the unchanged base (keys and values); paired-stem initialization, absent-goal and camera-batch equivalences; late tokens, identity and mask correctness, separate encoder; identity FiLM on the goal pass; regime validation; bit-exact train/resume and standalone serving for both fusions (task-id invariance, goal sensitivity, fixed goals); and a small policy that learns two goals from one observation with the zero inference latent (both fusions). Full suite on 2026-09-20: 137 passed, 5 skipped (CUDA-only, real-encoder and live-upload opt-ins).
