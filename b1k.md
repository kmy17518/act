...
All of the following keep fp32 weights, fp32 optimizer state, the upstream loss, the same trained/frozen parameters and the same sampling; they change kernels, layouts and precision of intermediate arithmetic only. Measured on one Blackwell GPU with 30 cores (`turning_on_radio`, 14–150 step probes, steady-state median): batch 1560 **3.40 → 0.48 s/step** and batch 1024 **1.77 → 0.32 s/step** with the radio recipe (`--frame-cache`, `--matmul-precision high`, `--compile regions`, defaults otherwise).
...
| `--compile backbone\|regions` | `none` | Inductor fuses the frozen-BN affine/ReLU/residual/pooling elementwise passes around cuDNN convolutions (`backbone`), plus the style encoder and transformer as separate graphs with the attention pattern matcher off (`regions`); compile takes a few minutes once (on-disk cache afterwards) | backbone rel Δl1 3e-7; regions rel Δloss 2.5e-6, gradient cosine 1.0; dropout masks come from Inductor's RNG (same distribution); resume reproduces the uninterrupted losses exactly |
| `--autocast bf16` | off | bf16 forward for the backbone/transformer with fp32 output heads, latent distribution and losses; fastest (batch 1024 0.29 s/step, batch 1560 0.44 s/step before compile) | **not numerically neutral**: rel Δloss 1e-4 on one batch, and resumed step-10000 L1 is systematically +0.6–1.0 % with a 20–50 % larger gradient norm; kept opt-in, not used by the radio recipe |
