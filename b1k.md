...
`scripts/b1k/run_radio_mt_act_300k.sh` is the launch recipe (96×96 images — the Diffusion Policy baseline's resolution — batch 1560, the radio optimizer settings). CPU coverage: `tests/test_b1k_language.py` (`ACT_TEST_REAL_MINILM=1` adds the cached real-encoder check against RoboAgent's first shipped vector).
